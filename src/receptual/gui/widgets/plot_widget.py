...
class PlotWidget(QWidget):
	...
	def _plot_activity(self):
		...
		self.processor.get_dimension_info('activity')
		...
		dimensions = [
			{
				...
				'max': len(time_values)
				- 1,  # Frame index (internally we still use indices)
		...
		if slice_dialog.exec():
			...
			plot_widget.plot(
				sliced_time, sliced_activity, pen={'color': 'b', 'width': 2}
			)
			...
			plot_widget.setTitle(
				f'Neural Activity (Time: {min_time_str}-{max_time_str}s)'
			)
	...
	def _plot_stimulus(self):
		...
		if slice_dialog.exec():
			...
			slice_indices = [
				slice(ranges[dim['name']][0], ranges[dim['name']][1] + 1)
			...
			if len(sliced_stimulus.shape) == 1:
				self._plot_1d_stimulus(
					sliced_stimulus, dim_info, value_info, range_info
				)
			elif len(sliced_stimulus.shape) == 2:
				self._create_3D_surface_plot(
					sliced_stimulus, dim_info, value_info, range_info=range_info
				)
			else:
				# Still higher-dimensional, use the dimension selector
				self._create_3D_surface_from_highD(
					sliced_stimulus, dim_info, value_info, range_info=range_info
				)
	...
	def _plot_1d_stimulus(self, stimulus_1d, dim_info, value_info, range_info=None):
		...
		if range_info:
			...
			plot_widget.setTitle(
				f'1D Stimulus ({dim_name}: {min_val}-{max_val}{unit_str}, {max_val - min_val + 1} points)'
			)
		else:
	...
	def _create_3D_surface_plot(
		...
		plot_widget.setCameraPosition(
			distance=2.0 * max(dim0_points, dim1_points), elevation=25, azimuth=45
		)
	...
	def _create_3D_surface_from_highD(
		self, stimulus, dim_info, value_info, range_info=None
	):
	...
	def _create_wireframe_mesh(self, data):
		...
		vertices = np.vstack([x_grid.flatten(), y_grid.flatten(), data.flatten()]).T
