...
class DimensionSelectorDialog(QDialog):
	def __init__(self, processor, parent=None, preselect_dim0=None):
		...
		if preselect_dim0 is not None and preselect_dim0 < self.ndims:
			...
			x_dim = (
				1
				if preselect_dim0 != 1 and self.ndims > 1
				else (0 if preselect_dim0 != 0 else 2)
			)
			...
			self.dim_y_combo.setCurrentIndex(
				self.dim_y_combo.findData(0)
			)  # First dimension for Y
			self.dim_x_combo.setCurrentIndex(
				self.dim_x_combo.findData(1 if self.ndims > 1 else 0)
			)  # Second for X
	...
	def update_slice_selectors(self):
		...
		for i in range(self.ndims):
			if i not in selected_dims:
				...
				unit_str = f' ({unit})' if unit else ''
				slice_layout.addWidget(
					QLabel(f'{self.dim_names[i]}{unit_str} slice [0-{max_idx}]:')
				)
	...
	def get_selection(self):
		...
		return {
			...
			'slices': {
				dim: spinner.value() for dim, spinner in self.slice_spinners.items()
			},
		}
