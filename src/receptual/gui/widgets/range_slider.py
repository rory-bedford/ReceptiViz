...
class SliceRangeDialog(QDialog):
	...
	def _setupUI(self):
		...
		info_label.setStyleSheet(
			'font-style: italic; color: gray; padding-bottom: 10px;'
		)
		...
		for i, dim in enumerate(self.dimensions):
			...
			if name.lower() == 'time' and unit == 's':
				...
				slider.rangeChanged.connect(
					...
					max_v,
					lbl=label,
					sr=sample_rate: self._update_time_display(lbl, min_v, max_v, sr)
				)
			else:
			...
			slider.rangeChanged.connect(
				lambda min_v, max_v, dim_name=name: self._update_point_count(
					dim_name, min_v, max_v
				)
			)
