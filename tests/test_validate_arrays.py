...
class TestReceptiveFieldValidation:
    ...
    def test_validate_incompatible_arrays(self, processor, incompatible_arrays):
        ...
        activity_valid, _, activity_data = processor.validate_file(str(activity_path), "activity")
        assert activity_valid is True
        processor.set_activity(activity_data)
        
        # Load stimulus with 100 timepoints
        stimulus_valid, errors, _ = processor.validate_file(str(stimulus_path), "stimulus")
    ...
    def test_validate_incompatible_arrays_reversed(self, processor, incompatible_arrays):
        activity_path, _, stimulus_path, stimulus_data = incompatible_arrays
        
        # First load stimulus with 100 timepoints
        stimulus_valid, _, stimulus_data = processor.validate_file(str(stimulus_path), "stimulus")
        ...
        activity_valid, errors, _ = processor.validate_file(str(activity_path), "activity")
